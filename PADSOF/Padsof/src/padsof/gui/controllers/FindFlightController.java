...
public class FindFlightController extends Controller<FindFlightView>
{
	...
	public class FlightInfoWrapper
	{
		...
		public String dateToString(Date date)
		{
			...
			return departure.get(Calendar.DAY_OF_MONTH) + "/" + (departure.get(Calendar.MONTH) + 1)+ " " 
					+ departure.get(Calendar.HOUR) + ":" + departure.get(Calendar.MINUTE);
		}
		
		public String toString()
		{			
			return info.getSource()  + " (" + dateToString(info.getDeparture()) + ") => " + 
					info.getDestination() + "(" + dateToString(info.getArrival()) + ") - " + info.getPrice() + "€";
		}
	...
	public void search()
	{
		...
		try
		{
			 List<String> flights = fp.queryFlights(origin.getCode(), destination.getCode(), start, end);
